...
## Publishing the profile_overlay image to Cloudflare KV

Given you have configured a KV binding named "MANDELATAR_ASSETS" in `wrangler.toml`:
```
# Push to production
wrangler kv:key put --binding="MANDELATAR_ASSETS" "profile_overlay_x" --path "mandelatar-core/assets/profile_overlay_x.png" --preview false

# Push to preview (for use in `wrangler dev`)
wrangler kv:key put --binding="MANDELATAR_ASSETS" "profile_overlay_x" --path "mandelatar-core/assets/profile_overlay_x.png" --preview
```
